...
interface AllotProfitDao {
    ...
    @Select("select * from lc_allot_profit where partner_id = #{partnerId}")
    fun getByPartnerId(@Param("partnerId") partnerId: String): List<AllotProfit>

    @Insert("insert into lc_allot_profit(allot_profit_id, partner_id, seal_wallet_addr, income_method, partner_ratio, seal_ratio, " +
    ...
    @Update("update lc_allot_profit set partner_id=#{allotProfit.partnerId}, seal_wallet_addr=#{allotProfit.sealWalletAddr}, income_method=#{allotProfit.incomeMethod}, " +
