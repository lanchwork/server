...
interface PartnerDao {
    ...
    @Select("select * from lc_partner where partner_id = #{partnerId}")
    fun getPartnerById(@Param("partnerId") partnerId: String): Partner

    @Insert("insert into lc_partner(partner_id, channel_mark, partner_name, user_no, wallet_addr, open_flag, create_date, create_user, update_date, update_user, remark) " +
    ...
    @Delete("delete from lc_partner where partner_Id = #{partnerId}")
    fun deletePartnerById(@Param("partnerId") partnerId: String): Int

    @Update("update lc_partner set channel_mark=#{partner.channelMark}, partner_name=#{partner.partnerName}, user_no=#{partner.userNo}, wallet_addr=#{partner.walletAddr}, open_flag=#{partner.isOpen}, update_date=#{partner.updateDate}, update_user=#{partner.updateUser}, remark=#{partner.remark} " +
            "where partner_id = #{partner.partnerId}")
    fun updatePartnerById(@Param("partner") partner: Partner): Int

    @Select("select * from lc_partner limit #{currentPage}, #{pageSize}")
    fun getPartnerList(@Param("currentPage") currentPage: Int, @Param("pageSize") pageSize: Int): List<Partner>

    @Select("select partner_id,channel_mark,partner_name from lc_partner where open_flag = 1")
