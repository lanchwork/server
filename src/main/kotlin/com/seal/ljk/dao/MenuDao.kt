...
interface MenuDao {
    ...
    @Insert("insert into menu(menu_name,menu_type)" +
            "values(#{menu.menuName},#{menu.menuType})")
    fun createMenu(@Param("menu") menu : Menu)

    @Delete("delete from menu where id = #{menuId}")
    fun deleteMenuById(menuId: String)

    @Update("update menu set menu_name=#{menu.menuName},menu_type=#{menu.menuType} where id = #{menu.id}")
