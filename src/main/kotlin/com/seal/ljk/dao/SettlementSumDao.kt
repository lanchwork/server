...
interface SettlementSumDao {
    ...
    @Select("select * from settlement_sum where user_no = #{userNo}")
    fun getSettlementSumByUser(@Param("userNo") userNo: String): SettlementSum?

    @Insert("insert into settlement_sum(settlement_sum_id, user_no, total_to_settle_amt, total_settled_amt, paid_profit, to_pay_profit, create_date, create_user, update_date, update_user, remark) " +
