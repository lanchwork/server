...
interface InvestSumDao {
    ...
    @Select("select * from lc_invest_sum where user_no = #{userNo}")
    fun getInvestSumByUser(@Param("userNo") userNo: String): InvestSum?

    @Insert("insert into lc_invest_sum(invest_sum_id, user_no, total_pending_amt, total_invest_amt, earned_amt, unearned_amt, create_date, create_user, update_date, update_user, remark) " +
